...
FuncDef[] aFunc = (FuncDef[])GLOBAL( FuncDef, aBuiltinFunc );
		...
		static void lnFunc(sqlite3_context context, int argc, sqlite3_value[] argv)
		{
			double r = sqlite3_value_double(argv[0]);
			r = Math.Log(r);
			sqlite3_result_double(context, r);
		...
		static void powFunc(sqlite3_context context, int argc, sqlite3_value[] argv)
		{
			if (argc != 2)
				return;
			double r = sqlite3_value_double(argv[0]), e = sqlite3_value_double(argv[1]);
			r = Math.Pow(r, e);
			sqlite3_result_double(context, r);
		}
		static void logFunc(sqlite3_context context, int argc, sqlite3_value[] argv)
		{
			double r = sqlite3_value_double(argv[0]), e = 10.0;
			if (argc == 2)
				e = sqlite3_value_double(argv[1]);
			r = Math.Log(r, e);
			sqlite3_result_double(context, r);
		}
		static void strposFunc(sqlite3_context context, int argc, sqlite3_value[] argv)
		{
			int r = 0;
			if (argc == 2)
			{
				string text = sqlite3_value_text(argv[0]);
				string search = sqlite3_value_text(argv[1]);
				r = text.IndexOf(search) + 1;
			}
			sqlite3_result_int(context, r);
		}
		static void newidFunc(sqlite3_context context, int argc, sqlite3_value[] argv)
		{
			var res = Guid.NewGuid().ToString();
			sqlite3_result_text(context, res, -1, null);
		}
		static void indexofFunc(sqlite3_context context, int argc, sqlite3_value[] argv)
		{
			int r = -1;
			if (argc == 2)
			{
				string target = sqlite3_value_text(argv[0]);
				string search = sqlite3_value_text(argv[1]);
				r = search.IndexOf(target) + 1;
			}
			sqlite3_result_int(context, r);
		}
	}
