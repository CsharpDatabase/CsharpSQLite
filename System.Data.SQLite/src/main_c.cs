...
*/
		...
		*/
		...
		string zName,
		u8 enc,
		CollatingSequenceTypes collType,
		object pCtx,
...
;
	  ...
	  */
			createCollation(db, "BINARY", SQLITE_UTF8, CollatingSequenceTypes.Binary, 0,
					 binCollFunc, null);
			createCollation(db, "BINARY", SQLITE_UTF16BE, CollatingSequenceTypes.Binary, 0,
					binCollFunc, null);
			createCollation(db, "BINARY", SQLITE_UTF16LE, CollatingSequenceTypes.Binary, 0,
					binCollFunc, null);
			createCollation(db, "RTRIM", SQLITE_UTF8, CollatingSequenceTypes.User, 1,
					binCollFunc, null);
			//if ( db.mallocFailed != 0 )
			//{
			//  goto opendb_out;
			...
			createCollation(db, "NOCASE", SQLITE_UTF8, CollatingSequenceTypes.Nocase, 0,
...
*/
		...
		)
		{
			...
			sqlite3_mutex_enter(db.mutex);
			//Debug.Assert( 0 == db.mallocFailed );
			rc = createCollation(db, zName, (u8)enc, CollatingSequenceTypes.User, pCtx, xCompare, null);
			rc = sqlite3ApiExit(db, rc);
			sqlite3_mutex_leave(db.mutex);
			return rc;
		}
		...
		*/
		...
		)
		{
			...
			sqlite3_mutex_enter(db.mutex);
			//Debug.Assert( 0 == db.mallocFailed );
			rc = createCollation(db, zName, (u8)enc, CollatingSequenceTypes.User, pCtx, xCompare, xDel);
			rc = sqlite3ApiExit(db, rc);
			sqlite3_mutex_leave(db.mutex);
			return rc;
		}
